 ...
 *
 * This page connects users to verified human financial advisors,
 * NOT the AI assistant. The AI assistant is accessible via "AI Advisor" in the nav.
 */
...
export default function Consult() {
  ...
  const [step, setStep] = useState(1); // 1=Expert, 2=Session+Time, 3=Details
  ...
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto pt-16 text-center">
        <div className="w-16 h-16 rounded-full bg-emerald-600/10 border border-emerald-500/30 flex items-center justify-center mx-auto">
          <Check className="w-8 h-8 text-emerald-400" />
        </div>
        <h2 className="mt-5 text-xl font-bold text-[var(--c-text)]">Booking Request Sent</h2>
        <p className="mt-2 text-sm text-[var(--c-muted)] leading-relaxed">
          Your consultation request has been received. <strong className="text-[var(--c-sub)]">{selectedExpert?.name}</strong> will
          confirm your slot via email within 2 business hours.
        </p>
        <div className="mt-6 rounded-xl border border-[var(--c-border)] bg-[var(--c-surface)] p-5 text-left space-y-2.5 text-sm">
          <div className="flex justify-between">
            <span className="text-[var(--c-muted)]">Advisor</span>
            <span className="font-medium text-[var(--c-text)]">{selectedExpert?.name}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[var(--c-muted)]">Session Type</span>
            <span className="font-medium text-[var(--c-text)]">{selectedSession?.title}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[var(--c-muted)]">Date</span>
            <span className="font-medium text-[var(--c-text)]">{selectedDate}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[var(--c-muted)]">Time Slot</span>
            <span className="font-medium text-[var(--c-text)]">{selectedSlot} IST</span>
          </div>
          <div className="flex justify-between">
            <span className="text-[var(--c-muted)]">Duration</span>
            <span className="font-medium text-[var(--c-text)]">{selectedSession?.duration}</span>
          </div>
        </div>
        <button
          onClick={() => { setSubmitted(false); setStep(1); setSelectedExpert(null); setSelectedSession(null); setSelectedDate(''); setSelectedSlot(''); setForm({ name: '', email: '', phone: '', notes: '' }); }}
          className="mt-6 px-5 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-sm transition-colors"
        >
      ...
      <div className="flex items-center gap-2 text-[12px] font-medium">
        {[
          ...
          <div key={s.n} className="flex items-center gap-2">
            <button
              onClick={() => { if (s.n <= step || (s.n === 2 && canProceed1) || (s.n === 3 && canProceed2)) setStep(s.n); }}
            ...
            {i < 2 && <ChevronRight className="w-3.5 h-3.5 text-[var(--c-dimmer)]" />}
          </div>
        ))}
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-7 max-w-xl">
          ...
          <div className="rounded-xl border border-[var(--c-border)] bg-[var(--c-surface)] p-5 space-y-2 text-sm">
            ...
            {[
              ...
              ['Registration', selectedExpert?.regNo],
          ...
          <div className="space-y-4">
            <h2 className="text-[16px] font-semibold text-[var(--c-text)]">Your Contact Details</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[12px] font-medium text-[var(--c-sub)]">Phone Number *</label>
                <div className="relative">
          ...
          <div className="flex items-start gap-2.5 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3.5">
            ...
            <p className="text-[11px] text-amber-300/80 leading-relaxed">
              This is a booking inquiry form. Sessions are subject to advisor confirmation. Payment
              is collected only after confirmation. Advisor credentials are independently verified
              with SEBI. This platform does not act as an intermediary or investment manager.
            </p>
          </div>

          <div className="flex justify-between pt-1">
            <button type="button" onClick={() => setStep(2)} className="px-4 py-2 rounded-lg border border-[var(--c-border)] text-sm text-[var(--c-muted)] hover:text-[var(--c-sub)] transition-colors">
              Back
            </button>
            <button
              type="submit"
